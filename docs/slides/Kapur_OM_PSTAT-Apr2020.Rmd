...
## .inverse[Up next: Conditioning & OM dynamics]
...
**Eq 17**
+ Baranov (Ricker, 1975): catches occur by fleets $f$ in sub-areas $i$, and accrue to mgmt regions $k$.
...
---
# Survey Biomass
$B_y^f={\epsilon_y^fq}^f\sum_{i}\sum_{a}{{\phi_{if}w_a^{k,f}s}_a^fN_{y,a}^i}$
...
$$L_\varphi^f=\frac{\mathrm{\Gamma}(n_y^f+1)}{\prod_{a}^{A}{\mathrm{\Gamma}(n_y^f\widetilde{\sum_{\gamma}\pi_{y,\gamma,a}^f}+1)}}\frac{\mathrm{\Gamma}(\theta^fn_y^f)}{\mathrm{\Gamma}(n_y^f+\theta^fn_y^f)} \\
...
<Br><Br><Br><Br><Br><Br>
# .inverse[Take a break?]
## .inverse[Up next: Reference points, and things I haven't thought of yet]
...
---
# Things I haven't gone through yet
+ .inverse[Aging Error]
+ .inverse[Expansions]
---
