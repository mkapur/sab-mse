  ...
  if("srv" %in% boundSlx){
    ...
    if(length(grep("log_srv_slx_pars", names(mappy)))  > 0){
      ...
      if(all(!is.na(map_srvslx['AK_VAST_W',,1,]))){
        ...
        upr.temp['AK_VAST_W',"p1",1:df$srv_blks_size[,'AK_VAST_W'],] <- log(41); 
        
        lwr.temp['AK_VAST_W',"p2",1:df$srv_blks_size[,'AK_VAST_W'],] <- log(44); 
        upr.temp['AK_VAST_W',"p2",1:df$srv_blks_size[,'AK_VAST_W'],] <- log(55)
      }
      if(all(!is.na(map_srvslx['AK_VAST_E',,1,]))){
        lwr.temp['AK_VAST_E',"p1",1:df$srv_blks_size[,'AK_VAST_E'],] <- log(22);
        upr.temp['AK_VAST_E',"p1",1:df$srv_blks_size[,'AK_VAST_E'],] <- log(41)
      ...
      if(all(!is.na(map_srvslx['WC_VAST',,1,]))){
        ...
        upr.temp['WC_VAST',"p1",1:df$srv_blks_size[,'WC_VAST'],] <- log(65)
