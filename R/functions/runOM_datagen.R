...
runOM_datagen <- function(df, seed = 731){
  ...
  # for(y in 1:3){
    ...
    if(y == 1){ 
      for(i in 1:nspace){
        for(s in 1:2){ ## sexes
          ...
          N_yais_mid[1,1,i,s] <- N_yais_beg[1,1,i,s]*exp(-mat_age[1]/2)
    ...
    for(s in 1:2){
      for(i in 1:nspace){
        N_yais_mid[y,1,i,s] <- N_yais_beg[y,1,i,s]*exp(-mat_age[1]/2)
  ...
  # for(y in 1:(tEnd-1)){
    ...
    # for(fish_flt in 1:nfleets_fish){
      ...
      if(is.na(Freal_yf[y, fish_flt])) stop("NA F ON year ",y,"\t"," fleet ",fish_flt,"\n")
